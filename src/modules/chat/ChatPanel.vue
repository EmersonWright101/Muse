...
import {
  Send, Square, Paperclip, MessageSquare, X, FileText, Eraser, SquarePen, Brain, Plus, Check, Globe, Columns2,
...
import type { AttachmentMeta, Conversation }  from '../../stores/chat'
import { processPdfFile }       from '../../utils/pdf'

const props = defineProps<{
  panelKey: 'left' | 'right' | 'window'
  windowConvId?: string
}>()
...
const isLeft = computed(() => props.panelKey === 'left')
const isWindow = computed(() => props.panelKey === 'window')
...
const conv = computed(() => {
  if (isWindow.value) return localWindowConv.value
  return isLeft.value ? chat.activeConv : chat.secondaryActiveConv
})

const activeAssistant = computed(() => {
  const id = conv.value?.assistantId
...
function commitTitleEdit() {
  ...
  if (trimmed && conv.value) {
    chat.renameConversation(conv.value.id, trimmed)
  }
...
const hasConv    = computed(() => !!conv.value)
const messages   = computed(() => conv.value?.messages ?? [])
const canSend    = computed(() => (inputText.value.trim() || pendingImages.value.length > 0) && !isPanelStreaming.value)

const isPanelStreaming = computed(() => {
  const id = convId.value
  return id ? chat.streamingConvIds.has(id) : false
})
...
async function send() {
  ...
  if (isLeft.value) {
    ...
    if (cid) await chat.sendMessageTo(cid, text, images.length ? images : undefined)
  } else {
    const cid = chat.secondaryActiveConvId
    if (cid) await chat.sendMessageTo(cid, text, images.length ? images : undefined)
  }
}
...
function startNewChat() {
  const assistantId = conv.value?.assistantId
...
<template>
  <div class="chat-panel" :class="{ 'right-panel': !isLeft }">
    ...
    <div v-if="!hasConv" class="empty-state">
      ...
      <h2 class="empty-title">{{ isWindow ? '加载对话中…' : isLeft ? '开始新对话' : '选择右侧对话' }}</h2>
      <p class="empty-sub">
        {{ isWindow ? '' : isLeft ? '从左侧选择一个对话，或开始一次新的聊天。' : '从左侧侧边栏选择一个对话在右侧显示。' }}
      </p>
      <button v-if="isLeft" class="new-chat-btn" @click="startNewChat()">
        新建对话
      </button>
    </div>
    ...
    <template v-else>
      ...
      <div class="conv-header" :data-tauri-drag-region="isWindow ? '' : undefined">
        ...
        <div class="conv-meta">
          ...
          <ModelSelector :compact="chat.splitView" drop-down @select="onModelSelect" />
          <button
            v-if="isLeft && !chat.splitView"
            class="split-toggle-header-btn"
            title="开启分屏"
            @click="chat.toggleSplitView()"
          >
            <Columns2 :size="13" />
          </button>
          <button
            ...
            class="split-toggle-header-btn"
            title="关闭左侧"
            @click="chat.closeLeftPanel()"
          >
            <X :size="13" />
          </button>
          <button
            v-if="!isLeft && !isWindow && chat.splitView"
            class="split-toggle-header-btn"
            title="关闭右侧"
            @click="chat.closeSecondaryConversation()"
          >
            <X :size="13" />
          </button>
      ...
      <div class="input-area">
        ...
        <div class="input-box">
          ...
          <div class="input-toolbar">
            ...
            <button
              v-if="isPanelStreaming"
              class="send-btn stop"
              title="停止生成"
              @click="chat.stopStreaming(convId ?? undefined)"
            >
              <Square :size="15" />
            </button>
            <button
...
/* Split toggle button in header */
.split-toggle-header-btn {
  width: 26px;
  height: 26px;
  border: none;
  background: transparent;
...
.split-toggle-header-btn:hover {
  background: rgba(0, 0, 0, 0.06);
  color: #3c3c43;
}

.split-toggle-header-btn.active {
  color: #223F79;
  background: rgba(34, 63, 121, 0.10);
}
</style>
