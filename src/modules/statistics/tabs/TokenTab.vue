...
import { Hash, Building2, Type } from 'lucide-vue-next'
...
const sortOptions: { key: SortBy; label: string; icon: typeof Hash }[] = [
  { key: 'tokens', label: 'Token', icon: Hash },
  ...
  <div class="token-tab">
    ...
    <template v-else>
      <div class="tab-header">
        <h2 class="tab-title">Token 用量</h2>
        <TimeRangeSelector />
      </div>

      <div class="cards-grid">
        <div class="mini-card">
          <div class="mini-label">总 Input Tokens</div>
          <div class="mini-value">{{ formatNumber(totalInput) }}</div>
        </div>
        <div class="mini-card">
          <div class="mini-label">总 Output Tokens</div>
          <div class="mini-value">{{ formatNumber(totalOutput) }}</div>
        </div>
      </div>
          ...
          <div v-for="d in daily" :key="d.date" class="col-item">
            <div class="col-stack">
              <div class="col-track">
                ...
                >
                  ...
                  <div class="segment-tooltip">
                    ...
                    <div class="tip-line">Total: {{ formatNumber(m.totalTokens) }}</div>
                    <div class="tip-line">In: {{ formatNumber(m.inputTokens) }} / Out: {{ formatNumber(m.outputTokens) }}</div>
                  </div>
                </div>
              </div>
              <div class="col-overlay">
                ...
                >{{ formatNumber(d.totalTokens) }}</div>
              </div>
            </div>
      ...
      <div class="section-card">
        ...
        <div class="model-list">
          <div v-for="m in stats.filteredSortedModelStats" :key="m.modelId" class="model-row">
            ...
            <div class="model-num">{{ formatNumber(m.totalTokens) }}</div>
          </div>
        </div>
      </div>
    </template>
