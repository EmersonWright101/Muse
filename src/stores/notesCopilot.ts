...
const LS_KEY             = 'muse-notes-copilot'
const LS_MODIFIED_AT_KEY = 'muse-notes-copilot-modified-at'

export const NOTES_COPILOT_SYSTEM_PROMPT = 'You are a writing assistant. Help the user continue their note. Output ONLY the NEW continuation text, do not repeat existing text.'
...
export const useNotesCopilotStore = defineStore('notesCopilot', () => {
  ...
  function save() {
    ...
    apiPut('/api/settings/notesCopilot', { value: payload }).catch(() => {})
  }
  ...
  async function pushToServer() {
    await apiPut('/api/settings/notesCopilot', {
      value: {
        enabled: enabled.value,
        providerId: providerId.value,
        modelId: modelId.value,
  ...
  async function syncFromServer(allSettings: Record<string, unknown>) {
    const s = allSettings.notesCopilot as Record<string, unknown> | undefined
    if (!s) return
