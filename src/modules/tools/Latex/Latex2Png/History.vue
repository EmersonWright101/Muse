  ...
  <div class="w-full max-w-6xl mx-auto space-y-6 pb-10 p-8">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <button
          @click="router.push('/tools/latex/latex2png')"
          class="p-2 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors"
        >
          <ArrowLeft class="w-4 h-4" />
        </button>
        <div>
          <h2 class="text-base font-semibold text-slate-800">{{ t('latex.history.title') }}</h2>
          <p class="text-xs text-slate-400 mt-0.5">{{ t('latex.history.subtitle', { count: historyRecords.length }) }}</p>
        </div>
      </div>
    ...
    <Transition name="fade">
      ...
      >
        <span class="text-sm text-red-700">{{ t('latex.history.confirm_clear') }}</span>
    ...
    <div v-if="historyRecords.length === 0" class="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p class="text-sm font-medium text-slate-500">{{ t('latex.history.empty_title') }}</p>
      <p class="text-xs text-slate-400 mt-1">{{ t('latex.history.empty_desc') }}</p>
    </div>
      ...
      <div v-for="group in groupedRecords" :key="group.dateKey">
        ...
        <TransitionGroup name="list" tag="div" class="space-y-2">
          <div
            ...
            class="group bg-white border border-slate-200 rounded-xl px-4 py-3 cursor-pointer hover:border-purple-300 hover:shadow-sm transition-all duration-200 flex items-start gap-3"
          >
            <!-- Resolution badge -->
            <div class="flex-shrink-0 mt-0.5">
              <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-semibold bg-purple-50 text-purple-600 border border-purple-100 whitespace-nowrap tabular-nums">
                {{ record.resolution }}x
              </span>
            </div>
            ...
            <div class="flex-1 min-w-0">
              <p class="text-xs text-slate-600 font-mono truncate">{{ record.latex }}</p>
            </div>
...
import {
  ...
  clearHistory,
  pendingRestore,
  type Latex2PngRecord,
} from '../../stores/latex2png';
...
function dateLabel(dateKey: string): string {
  if (dateKey === todayKey()) return t('latex.history.date_today');
  if (dateKey === yesterdayKey()) return t('latex.history.date_yesterday');
...
const groupedRecords = computed<DateGroup[]>(() => {
  const map = new Map<string, Latex2PngRecord[]>();
...
function restoreRecord(record: Latex2PngRecord) {
  pendingRestore.value = { latex: record.latex, resolution: record.resolution };
  router.push('/tools/latex/latex2png');
}
