  ...
  <div class="w-full max-w-6xl mx-auto space-y-6 pb-10 p-8">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <button
          @click="router.push('/tools/text/stats')"
          class="p-2 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors"
        >
          <ArrowLeft class="w-4 h-4" />
        </button>
        <div>
          <h2 class="text-base font-semibold text-slate-800">{{ t('stats.history.title') }}</h2>
          <p class="text-xs text-slate-400 mt-0.5">{{ t('stats.history.subtitle', { count: historyRecords.length }) }}</p>
        </div>
      </div>
    ...
    <Transition name="fade">
      ...
      >
        <span class="text-sm text-red-700">{{ t('stats.history.confirm_clear') }}</span>
    ...
    <div v-if="historyRecords.length === 0" class="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p class="text-sm font-medium text-slate-500">{{ t('stats.history.empty_title') }}</p>
      <p class="text-xs text-slate-400 mt-1">{{ t('stats.history.empty_desc') }}</p>
    </div>
      ...
      <div v-for="group in groupedRecords" :key="group.dateKey">
        ...
        <TransitionGroup name="list" tag="div" class="space-y-2">
          <div
            ...
            class="group bg-white border border-slate-200 rounded-xl px-4 py-3 cursor-pointer hover:border-emerald-300 hover:shadow-sm transition-all duration-200 flex items-start gap-3"
          >
            <!-- Stats chips -->
            <div class="flex-shrink-0 mt-0.5 flex flex-col gap-1">
              <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold bg-emerald-50 text-emerald-600 border border-emerald-100 whitespace-nowrap tabular-nums">
                {{ record.charCount.toLocaleString() }} {{ t('stats.chars_unit') }}
              </span>
              <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold bg-indigo-50 text-indigo-600 border border-indigo-100 whitespace-nowrap tabular-nums">
                {{ record.wordCount.toLocaleString() }} {{ t('stats.words') }}
              </span>
            </div>

            <!-- Text preview -->
            <div class="flex-1 min-w-0 space-y-1.5">
              <span class="text-sm text-slate-700 line-clamp-2 break-all">{{ record.inputText }}</span>
              <div class="flex items-center gap-3 text-[11px] text-slate-400">
                <span>{{ record.sentences }} {{ t('stats.sentences') }}</span>
                <span>·</span>
                <span>{{ record.paragraphs }} {{ t('stats.paragraphs') }}</span>
                <span>·</span>
                <span>{{ record.readingTimeStr }}</span>
              </div>
            </div>
...
import {
  ...
  deleteHistoryRecord,
  clearHistory,
  type TextStatsRecord,
} from '../../stores/textStats';
...
function dateLabel(dateKey: string): string {
  if (dateKey === todayKey()) return t('stats.history.date_today');
  if (dateKey === yesterdayKey()) return t('stats.history.date_yesterday');
...
const groupedRecords = computed<DateGroup[]>(() => {
  const map = new Map<string, TextStatsRecord[]>();
